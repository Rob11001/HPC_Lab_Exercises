 ...
 * @brief 
 * 
 * Adds only even elements of two array of N floating point numbers using intrinsic operations (maskload)
 */
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < N ; i += 8) {
        __m256 a_v = _mm256_maskload_ps(&a[i], mask);
        __m256 b_v = _mm256_maskload_ps(&b[i], mask);
        
        __m256 c_v = _mm256_add_ps(a_v, b_v);
        float *tmp = &c[i];
        _mm256_stream_ps(tmp, c_v);
    }
